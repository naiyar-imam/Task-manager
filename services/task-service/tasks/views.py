...
from .serializers import TaskCalendarSyncSerializer, TaskSerializer
...
class HealthView(APIView):
    ...
    def get(self, request):
        return Response({"status": "ok", "service": "task-service"})
...
class TaskQueryMixin:
    ...
    def filter_queryset(self, queryset):
        ...
        if tab == "today":
            ...
            queryset = queryset.filter(due_date__lt=now).exclude(
                status=Task.Status.COMPLETED
            )
...
class TaskExportView(APIView):
    def get(self, request):
        queryset = Task.objects.filter(user_id=request.user.id).order_by(
            "due_date",
            "-created_at",
        )
...
class TaskCalendarSyncDetailView(APIView):
    def patch(self, request, pk):
        task = generics.get_object_or_404(Task, pk=pk, user_id=request.user.id)
        serializer = TaskCalendarSyncSerializer(data=request.data, partial=True)
...
class TaskCalendarSyncClearView(APIView):
    def post(self, request):
        cleared = Task.objects.filter(user_id=request.user.id).update(
            ...
            trend.append(
                {
                    ...
                    "due": queryset.filter(due_date__date=current_date).count(),
                    "overdue": queryset.filter(due_date__lt=current_end)
